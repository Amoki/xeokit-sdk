...
class TilesetLoaderPlugin extends Plugin {
   ...
   * @param { number } [cfg.sensitivity=50] The distance from the camera used to compute the tiles visibility.
   * @param { number } [cfg.distanceFactorToFreeData=3] The distance from the camera used to free tiles data.
   * @param { boolean } [cfg.debugMode=false] If `true`, Tiles bounding box are shown.
   * 
  ...
  constructor(viewer, cfg = {}) {
    ...
    this.distanceFactorToFreeData = distanceFactorToFreeData;
    this.computePriority = computePriority;
    this.debugMode = debugMode;

    this.tilesets = new Set();

    viewer.camera.on("matrix", this.updateTilesetsVisibilityCallback = () => this.updateTilesetsVisibility());

    viewer.scene.on("tick", this.renderTilesetsCallback = () => this.renderTilesets());
  ...
  set sensitivity(value) {
    ...
    this.tilesets.forEach(tileset => tileset.sensitivity = this._sensitivity)
  }
   ...
   */
  destroy() {
    this.viewer.camera.off(this.updateTilesetsVisibilityCallback);

    this.viewer.scene.off(this.renderTilesetsCallback);
